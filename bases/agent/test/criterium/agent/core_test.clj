...
(deftest pid-test
  ;; PID extraction from runtime MXBean
         (testing "pid"
                  (testing "returns a numeric string"
                           (let [result (agent-core/pid)]
                                (is (string? result)
                                    "PID should be a string")
                                (is (re-matches #"\d+" result)
                                    "PID should be all digits")
                                (is (pos? (Integer/parseInt result))
                                    "PID should be positive")))))

(deftest agent-path-test
  ;; Agent path extraction
         (testing "agent-path"
                  (testing "returns nil when loader fails"
                           (with-redefs [criterium.agent.loader/extract-agent (fn [] (throw (Exception. "Test failure")))]
                                        (is (nil? (agent-core/agent-path))
                                            "Should return nil on extraction failure")))

                  (testing "returns path when extraction succeeds"
                           (with-redefs [criterium.agent.loader/extract-agent (constantly "/tmp/test-agent.so")]
                                        (is (= "/tmp/test-agent.so" (agent-core/agent-path))
                                            "Should return path from loader")))))

(deftest load-agent-test
  ;; Programmatic agent loading
         (testing "load-agent!"
                  (testing "throws IllegalStateException when already attached"
                           (with-redefs [agent-core/attached? (constantly true)]
                                        (is (thrown? IllegalStateException (agent-core/load-agent!))
                                            "Should throw when agent already loaded")))

                  (testing "throws RuntimeException when platform unsupported"
                           (with-redefs [agent-core/attached? (constantly false)
                                         agent-core/agent-path (constantly nil)]
                                        (is (thrown? RuntimeException (agent-core/load-agent!))
                                            "Should throw when agent path unavailable")))

                  (testing "throws RuntimeException when VirtualMachine.attach fails"
                           (with-redefs [agent-core/attached? (constantly false)
                                         agent-core/agent-path (constantly "/tmp/test-agent.so")
                                         agent-core/pid (constantly "12345")]
                                        (is (thrown? RuntimeException (agent-core/load-agent!))
                                            "Should throw when attach fails")))))
