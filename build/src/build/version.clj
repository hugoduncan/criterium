...
(defn- expand-version-string
  "Expand a single version string, resolving dynamic components."
  [version-str]
  (-> version-str
      interpolate-version
      version->version-map
...
(defn- expand-pom-deps
  "Expand version strings in pom-deps map."
  [pom-deps]
  (when pom-deps
    (reduce-kv
     (fn [m lib coord]
       (assoc m lib
              (if-let [v (:mvn/version coord)]
                (assoc coord :mvn/version (expand-version-string v))
                coord)))
     {}
     pom-deps)))
...
(defn expand-version
  "Expand version string in params map, resolving dynamic components like {{git-rev-count}}.
      Also expands versions in :pom-deps if present."
  [params]
  (let [version (expand-version-string (:version params))]
    (cond-> (assoc params :version version)
      (:pom-deps params)
      (update :pom-deps expand-pom-deps))))
