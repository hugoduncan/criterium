...
(defn anomalous-lags
  ...
  [lag-severities]
  (->> lag-severities
       (remove (fn [[_ sev]] (#{:none :alternating-none} sev)))
       (map first)
       sort
       vec))
...
(defn detect-pattern
  ...
  (let [floor (noise-floor n)
        ...
        is-drift? (and (> r-drift drift-threshold)
                       (not= lag-1-sev :none))]
    (cond
      ;; Negative lag-1 - return severity-qualified alternating pattern
      ;; Check this before all-clean to ensure alternating is reported
      (neg? r1)
      lag-1-sev
    ...
    (when-let [acf-map (acf samples)]
      (let [r1 (double (get acf-map 1 0.0))
        ...
        {:acf acf-map
         ...
         :lag-severities lag-sevs
         :anomalous-lags (anomalous-lags lag-sevs)
         :effective-sample-size {:n-original n}}))))
