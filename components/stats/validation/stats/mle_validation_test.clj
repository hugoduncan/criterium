      ...
      (do
        (testing "against R's MASS::fitdistr"
          (let [data-str (str "c(" (clojure.string/join "," gamma-test-data) ")")
                ...
                clj-result (stats/gamma-mle gamma-test-data)
          ...
          (doseq [[desc data expected-shape expected-scale]
            ...
            (testing desc
              (let [data-str (str "c(" (clojure.string/join "," data) ")")
                    ...
                    r-rate (second r-result)
                    r-scale (/ 1.0 r-rate)
                    clj-result (stats/gamma-mle data)
                    clj-shape (get-in clj-result [:params :shape])
                    clj-scale (get-in clj-result [:params :scale])]
      ...
      (do
        (testing "against R's MASS::fitdistr"
          (let [data-str (str "c(" (clojure.string/join "," lognormal-test-data) ")")
                ...
                r-loglik (nth r-result 2)
                clj-result (stats/lognormal-mle lognormal-test-data)
        ...
        (testing "closed-form matches direct calculation"
          ...
          (let [log-data (mapv #(Math/log %) lognormal-test-data)
                ...
                result (stats/lognormal-mle lognormal-test-data)]
          ...
          (doseq [[desc data]
            ...
            (testing desc
              (let [data-str (str "c(" (clojure.string/join "," data) ")")
                    ...
                    r-mu (first r-result)
                    r-lambda (second r-result)
                    clj-result (stats/inverse-gaussian-mle data)]
...
(deftest mle-aic-consistency-test
  ...
  (testing "MLE log-likelihood"
    (testing "can be used with AIC function"
      (let [gamma-result (stats/gamma-mle gamma-test-data)
    ...
    (testing "can be used with BIC function"
      (let [lognormal-result (stats/lognormal-mle lognormal-test-data)
    ...
    (testing "can be used with AICc function"
      (let [ig-result (stats/inverse-gaussian-mle inverse-gaussian-test-data)
