...
(def *zignor-v* 9.91256303526217e-3)
...
(defn zignor-init
  ...
  (let [c (int c)
    ...
    (loop [i (int 2)
	   f f]
      (aset s-adzigx i (Math/sqrt (* -2e0 (Math/log (+ (/ v (aget s-adzigx (dec i))) f)))))
      (when (< i c)
	(recur (inc i) (Math/exp (* -0.5e0 (aget s-adzigx i) (aget s-adzigx i))))))
...
"
  ([] (random-normal-zig (criterium.well/well-rng-1024a) (zignor-init *zignor-c* *zignor-r* *zignor-v*)))
  ([rng-seq] (random-normal-zig rng-seq (zignor-init *zignor-c* *zignor-r* *zignor-v*)))
  ([rng-seq c r v] (random-normal-zig rng-seq (zignor-init c r v)))
  ([c r v] (random-normal-zig (criterium.well/well-rng-1024a) (zignor-init c r v)))
     ...
     (letfn [(random-normal-tail
       ...
       (let [[deviate rng-seq]
	     (loop [rng-seq rng-seq]
	       (let [r  (first rng-seq)
		     u  (double (- (* 2e0 r) 1e0))
		     i  (bit-and (int (* Integer/MAX_VALUE (first (drop 1 rng-seq)))) mask)]
		 ...
		 (if (< (Math/abs u) (nth s-adzigr i))
		   ...
		   (if (zero? i)
		     ...
		     (let [x (* u (nth s-adzigx i))
			   f0 (Math/exp (* -0.5e0 (- (Math/pow (nth s-adzigx i) 2) (sqr x))))
			   f1 (Math/exp (* -0.5e0 (- (Math/pow (nth s-adzigx (inc i)) 2) (sqr x))))]
		       (if  (< (+ f1 (* (first (drop 2 rng-seq) ) (- f0 f1))) 1.0)
			 [x (drop 3 rng-seq)]
